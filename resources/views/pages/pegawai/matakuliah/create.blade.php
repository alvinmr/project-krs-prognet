...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('pegawai.matakuliah.store') }}" method="post">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input name="jumlah_sks" id="jumlah_sks" class="form-control @error('jumlah_sks') is-invalid @enderror"
                        placeholder="Contoh : 2" type="number" value="{{ old('jumlah_sks') }}"></input>
                    @error('jumlah_sks')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="time" class="form-control @error('jam_mulai') is-invalid @enderror" id="jam_mulai"
                        placeholder="Contoh : 198507232020072001" name="jam_mulai" value="{{ old('jam_mulai') }}">
                    @error('jam_mulai')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="time" class="form-control @error('jam_selesai') is-invalid @enderror" id="jam_selesai"
                        placeholder="Contoh : 198507232020072001" name="jam_selesai" value="{{ old('jam_selesai') }}">
                    @error('jam_selesai')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select name="dosen" class="form-control @error('dosen') is-invalid @enderror" id="dosen">
                        ...
                        @foreach ($dosen as $item)
                            <option value="{{ $item->id }}" {{ old('dosen') == $item->id ? 'selected' : '' }}>
                                {{ $item->nama }}</option>
                        @endforeach
                    </select>
                    @error('dosen')
                    ...
                    <select name="program_studi" class="form-control @error('program_studi') is-invalid @enderror"
                        ...
                        @foreach ($program_studi as $item)
                            <option value="{{ $item->id }}"
                                {{ old('program_studi') == $item->id ? 'selected' : '' }}>
